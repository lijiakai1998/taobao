...
//        Integer total=(Integer) session.getAttribute("selectTotal");
        goods goods=null;
        List<goods> list=new ArrayList<>();
        String[] aa=(String[]) session.getAttribute("allgoodsid");
        System.out.println(Arrays.toString(aa));
        for (int i=0;i<aa.length;i++){
            goods=weitaoService.buy1(Integer.parseInt(aa[i]));
            list.add(goods);//得到所有商品的集合
        }
        Integer selectTotal1=Integer.parseInt((String) session.getAttribute("selectTotal"));
        session.setAttribute("selectTotal1",selectTotal1);
...
//        goods.setgPrice(goods.getgPrice()*selectTotal1);
        Date vNow = new Date();
        String sNow = "";
        sNow += vNow.getYear();
    ...
    public String insert1(@RequestParam("WIDout_trade_no") String dDingdanhao, @RequestParam("WIDtotal_amount") double dJine,
                          @RequestParam("WIDsubject") String dname, @RequestParam("WIDbody")String nn,
                          HttpServletResponse response, HttpServletRequest request, HttpSession session)
            throws Exception{
        dingdan dingdan=new dingdan();
        Integer dUid=(Integer) request.getSession().getAttribute("uid");
        String[] aa=(String[]) session.getAttribute("allgoodsid");
        String[] count=(String[]) session.getAttribute("everygoodscount");
        for (int i=0;i<aa.length;i++){
            for (int n=i;n<=i;n++){
                dingdan.setdUid(dUid);
                dingdan.setdGid(Integer.parseInt(aa[i]));
                dingdan.setdDingdanhao(dDingdanhao);
                dingdan.setdShuliang(Integer.parseInt(count[n]));
                dingdan.setdZhuangtai(0);
                weitaoService.insert1(dingdan);
            }
        }
        request.getSession().setAttribute("dDingdanhao",dDingdanhao);
